...
public class NocturneInitializer implements ServletContainerInitializer {
    ...
    public void onStartup(Set<Class<?>> classes, ServletContext ctx) throws ServletException {
        ...
        NocturneActionMap actionMap = new NocturneActionMap();

        Map<String, Class<?>> actions = actionMap.getInstance();
        for (Class<?> c : classes) {
            if (!Perform.class.isAssignableFrom(c)) {
                if (logger.isLoggable(Level.FINE))
                    logger.log(Level.FINE, "{0} did not implement Perform; not registering", c.getName());
                continue;
            }
            if (logger.isLoggable(Level.FINE))
        ...
        ctx.setAttribute("mappings", actionMap);
    }
